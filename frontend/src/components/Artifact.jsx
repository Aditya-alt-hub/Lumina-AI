import { Check, Code2, Copy, EyeIcon, PanelRightClose, PanelRightOpen, X } from 'lucide-react'
...
<script>
  ...
  //we will create function for detect language
      ...
      <>
       {!collapsed ? <div className='flex flex-col h-full bg-[#0d0f14]'>
          <div className='h-16 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0'>
              <button 
              // className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0'onClick={()=>setCollapsed(true)}
              className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0'  onClick={() => {
    if (onClose) {
      onClose();
    } else {
      setCollapsed(true);
              ...
              {canPreview && <div className='flex items-center gap-1 bg-white/[0.04] border border-white/[0.06] p-1 rounded-lg'>
                ...
                ${
                  tab === "code"
                  ? "bg-gradient-to-br from-slate-600 to-slate-800 text-white border border-slate-500/30 shadow-sm"
          ...
          {tab==="code" && <div className='h-auto text-white flex border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0'>
            {
              artifacts[0]?.files?.map((f,index)=>(
                ...
                className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-all duration-150 border-r border-white/[0.05] relative cursor-pointer ${
                activeFile === index
                ...
                >
                  {f?.name}
                  {activeFile===index && <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-slate-500 via-slate-300 to-slate-500 rounded-t-full" /> }
  ...
  return (
    ...
    className="lg:hidden fixed bottom-24 right-4 z-40 flex items-center gap-2 px-3.5 py-2 rounded-xl bg-gradient-to-br from-slate-600 to-slate-800 hover:from-slate-500 hover:to-slate-700 text-white text-[12px] font-medium shadow-lg shadow-slate-500/20 border border-slate-500/20 cursor-pointer transition-all duration-150"
    >
      <Code2/>
      View Code
    </button>
    ...
    className='hidden lg:flex h-full border-l border-white/[0.06] flex-col overflow-hidden shrink-0 w-[270px]'>
